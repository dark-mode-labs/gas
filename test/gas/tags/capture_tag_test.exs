defmodule Gas.Tags.CaptureTagTest do
  use ExUnit.Case, async: true
  alias Gas.Tags.CaptureTag
      ...
      assert parse(template) ==
               {:ok,
                %CaptureTag{
                  ...
                  body: [
                    %Gas.Text{loc: %Loc{column: 19, line: 1}, text: " "},
                    %Gas.Object{
                    ...
                    %Gas.Text{loc: %Loc{column: 28, line: 1}, text: " "}
                  ]
  ...
  describe "Renderable impl" do
    test "capture captures" do
      ...
      context = %Gas.Context{vars: %{"yo" => "HEY!"}}

      {:ok, tag, _rest} = parse(template)

      assert Gas.Renderable.render(tag, context, []) ==
               {[], %Gas.Context{vars: %{"yo" => "HEY!", "var1" => "HEY! captured"}}}
    end
