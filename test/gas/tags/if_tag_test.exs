defmodule Gas.Tags.IfTagTest do
               ...
               {:ok,
                %IfTag{
                  ...
                  body: [
                    ...
                    %Gas.Text{loc: %Loc{column: 48, line: 1}, text: " "}
                  ],
                  else_body: [],
      ...
      assert parse(template) ==
               {
                 ...
                 %IfTag{
                   ...
                   else_body: [
                     %Gas.Text{loc: %Loc{column: 51, line: 1}, text: "else"}
                   ],
                   elsifs: [
                     {
                       %Gas.BinaryCondition{
  ...
  describe "Renderable impl" do
    test "if clause" do
      ...
      assert {[%Gas.Text{text: " if "}], ^context} = Renderable.render(tag, context, [])
    end
    ...
    test "unless clause" do
      ...
      assert {[%Gas.Text{text: " unless "}], ^context} = Renderable.render(tag, context, [])
    end
    ...
    test "else clause" do
      ...
      assert {[%Gas.Text{text: " else "}], ^context} = Renderable.render(tag, context, [])
    end
    ...
    test "elsif clause" do
      ...
      assert {[%Gas.Text{text: " elsif3 "}], ^context} = Renderable.render(tag, context, [])
    end
  end
end
