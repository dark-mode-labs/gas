defmodule Gas.ArgumentTest do
  ...
  describe "get/3" do
    test "basic var" do
      ...
      context = %Gas.Context{vars: %{"key" => 123}}
    ...
    test "nested vars" do
      ...
      context = %Gas.Context{vars: %{"key1" => %{"key2" => 123}}}
      assert {:ok, 123, ^context} = Argument.get(arg, context, [])
    end
    ...
    test "array access" do
      ...
      context = %Gas.Context{vars: %{"key" => [1, [1, 2, 3], 3]}}
    ...
    test "true value" do
      arg = %Variable{original_name: "value", loc: @loc, identifier: "value", accesses: []}
      context = %Gas.Context{vars: %{"value" => true}}
      assert {:ok, true, ^context} = Argument.get(arg, context, [])
    end

    test "false value" do
      arg = %Variable{original_name: "value", loc: @loc, identifier: "value", accesses: []}
      context = %Gas.Context{vars: %{"value" => false}}
  ...
  describe "get/3 with filters" do
    test "basic filter" do
      ...
      context = %Gas.Context{vars: %{"key" => nil}}
    ...
    test "missing filter" do
      ...
      assert context.errors == [%Gas.UndefinedFilterError{filter: "unknown", loc: @loc}]
    end
