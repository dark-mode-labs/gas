defmodule Gas.Parser do
  ...
  alias Gas.{Object, Lexer, Tag, Text}

  @type errors :: [{binary, Lexer.loc()}]
  @type entry :: Text.t() | Object.t() | Gas.Renderable.t()
  @type parse_tree :: [entry]

  @doc """
  Parse text

  It accepts a :tags option to specify a map of tags to use. See `Gas.Tag.default_tags/0`
  """
  ...
  @doc """
  Remove blank text when the entries account for a "blank body". Check `Gas.Block.blank?/1` protocol
  and the implementations
  """
  @spec remove_blank_text_if_blank_body(parse_tree) :: parse_tree
  def remove_blank_text_if_blank_body(entries) do
    if Gas.Block.blank?(entries) do
      Enum.reject(entries, &match?(%Gas.Text{}, &1))
    else
      entries
    end
  end
