defmodule Gas.ConditionExpression do
  ...
  def parse(tokens) do
    with {:ok, first_argument, rest} <- Argument.parse(tokens) do
      case rest do
        [{:end, _}] ->
          {:ok, %Gas.UnaryCondition{argument: first_argument, loc: first_argument.loc}}
        ...
        [{:comparison, _, operator} | rest] ->
          with {:ok, second_argument, rest} <- Argument.parse(rest) do
            case rest do
              ...
              _ ->
                {:error, "Expected condition", Gas.Parser.meta_head(rest)}
            end
          end
  ...
  defp build_error(reason, loc), do: %Gas.ArgumentError{message: reason, loc: loc}
end
