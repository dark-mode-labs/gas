defmodule Gas.FileSystem do
  ...
  ```elixir
  file_system = Gas.LocalFileSystem.new(template_path)
  text = Gas.render(template, file_system: {Gas.LocalFileSystem, file_system})
  ```

  This will render the template with a LocalFileSystem implementation rooted at 'template_path'.
  """

  # Called by Gas to retrieve a template file
...
defmodule Gas.BlankFileSystem do
  @moduledoc """
  ...
  def read_template_file(template_path, _opts) do
    {:error,
     %Gas.FileSystem.Error{
       reason: "This Gas context does not allow includes #{template_path}."
     }}
  end
end

defmodule Gas.PassThroughFileSystem do
  @moduledoc """
  Default file system that return error on call
  """
  @behaviour Gas.FileSystem
...
defmodule Gas.LocalFileSystem do
  @moduledoc """
