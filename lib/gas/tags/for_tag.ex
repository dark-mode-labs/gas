defmodule Gas.Tags.ForTag do
  ...
  defimpl Gas.Renderable do
    def render(tag, context, options) do
      ...
      with {:ok, enumerable, context} <- enumerable(tag.enumerable, context, options),
        ...
        {:error, message, context} ->
          exception = %Gas.ArgumentError{loc: tag.loc, message: message}
    ...
    defp do_for(
           enumerable,
           %Gas.Tags.ForTag{variable: variable} = tag,
           for_name,
           context,
           options
         ) do
      ...
      try do
        {result, context} =
          ...
          |> Enum.reduce({[], context}, fn {v, index}, {acc_result, acc_context} ->
            ...
            try do
              {result, acc_context} = Gas.render(tag.body, acc_context, options)
