defmodule Gas.Tags.CounterTag do
  alias Gas.Argument

  @type t :: %__MODULE__{
          loc: Gas.Parser.Loc.t(),
          argument: Argument.t(),
          operation: :increment | :decrement
        }

  @enforce_keys [:loc, :argument, :operation]
  defstruct [:loc, :argument, :operation]

  @behaviour Gas.Tag

  @impl true
  def parse(counter, loc, context) when counter in ~w(increment decrement) do
    with {:ok, tokens, context} <- Gas.Lexer.tokenize_tag_end(context),
         {:ok, argument, [{:end, _}]} <- Argument.parse(tokens) do
      {:ok, %__MODULE__{loc: loc, argument: argument, operation: String.to_atom(counter)},
       context}
    else
      {:ok, _var, rest} ->
        {:error, "Unexpected token after argument", Gas.Parser.meta_head(rest)}

      {:error, reason, _, loc} ->
        {:error, reason, loc}

      {:error, reason, loc} ->
        {:error, reason, loc}
    end
  end

  defimpl Gas.Renderable do
    def render(%Gas.Tags.CounterTag{operation: :increment} = tag, context, _options) do
      key = to_string(tag.argument)
      value = Gas.Context.get_counter(context, [key])

      value = value || 0

      context = %{context | counter_vars: Map.put(context.counter_vars, key, value + 1)}

      {[to_string(value)], context}
    end

    def render(%Gas.Tags.CounterTag{operation: :decrement} = tag, context, _options) do
      key = to_string(tag.argument)
      value = Gas.Context.get_counter(context, [key])

      value = (value || 0) - 1

      context = %{context | counter_vars: Map.put(context.counter_vars, key, value)}

      {[to_string(value)], context}
    end
  end
end
