defmodule Gas.Sigil do
  @moduledoc """
  Provides the `~LIQUID` sigil for validating and compiling Liquid templates using Gas.

  This sigil validates the template at compile time and returns a compiled Gas template.
  If the template has syntax errors, it will raise a CompileError with detailed information.

  ## Examples

      iex> import Gas.Sigil
      iex> template = ~LIQUID\"\"\"
      ...> Hello, {{ name }}!
      ...> \"\"\"
      iex> Gas.render(template, %{"name" => "World"})
      {:ok, "Hello, World!"}
  ...
  An optional module attribute @liquid_tags can set which tags will be used while parsing.
      ...
      defmodule MyModule do
        ...
        @liquid_tags Gas.Tag.default_tags() |> Map.put("current_line", CustomTags.CurrentLine)
  ...
  # Custom sigil for validating and compiling Liquid templates using Gas
  defmacro sigil_LIQUID({:<<>>, _meta, [string]}, _modifiers) do
    ...
    try do
      ...
      parsed_template = Gas.parse!(string, opts)

      # Return the parsed template
      Macro.escape(parsed_template)
    rescue
      e in Gas.TemplateError ->
